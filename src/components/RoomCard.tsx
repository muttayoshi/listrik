...
export default function RoomCard({ room, devices, settings, onAddDevice, onEditDevice, onEditRoom }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      {/* Room header */}
      <button
        className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50/60 transition-colors"
        onClick={() => setOpen((o) => !o)}
      >
        ...
        <div className="text-right flex-shrink-0 mr-1">
          <p className="font-bold text-emerald-700 text-sm" style={{ fontFamily: 'var(--font-display)' }}>
        ...
        <button
          ...
          className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-300 hover:text-gray-500 transition-colors flex-shrink-0"
        ...
        <div className="border-t border-gray-100">
          {sorted.length === 0 ? (
            <div className="px-4 py-5 text-center">
              <p className="text-sm text-gray-400 mb-3">Belum ada perangkat di ruangan ini</p>
              <button
                onClick={() => onAddDevice(room.id)}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 border border-emerald-200 px-3 py-2 rounded-full hover:bg-emerald-50 transition-colors"
            ...
            <div className="p-3 space-y-2">
              ...
              <button
                onClick={() => onAddDevice(room.id)}
                className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-emerald-200 text-xs font-semibold text-emerald-500 hover:bg-emerald-50 hover:border-emerald-300 transition-all"
                style={{ fontFamily: 'var(--font-display)' }}
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                  <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
